...
async function prepareBlogPosts({
  ...
  for (let contentFile of postFiles) {
    ...
    const postMeta = {
      title,
      date,
      permalink: path.join('/', siteConfig.repoName, nestedPostDir, fileName),
      externalUrl,
      html: postHtml
    };
...
async function prepareHome(
  posts: PostType[],
  { themePath, outputDir, siteConfig }: FileThemeConfig
) {
...
async function copyStaticAssets({
  outputDir,
  repoPath
}: Pick<RootFileThemeConfig, 'outputDir' | 'repoPath'>) {
